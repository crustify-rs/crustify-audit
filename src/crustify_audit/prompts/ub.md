...
**Getting the crate to build is your job**, and usually the first one. The
image gives you a C toolchain — cmake, ninja, nasm, yasm, autotools, clang,
git, curl — and nothing target-specific. The C library the crate wraps, and
anything else the tree wants, you install. Two things worth knowing before you
start guessing:

- the C sources are in the repo, beside the Rust. You do not need to fetch
  them.
- a crate under `crustify/rust/` was generated by crustify, and its manifests
  reference `ffibox` at the absolute path `/opt/ffibox`. Cargo cannot load the
  workspace until something is there: `git clone
  https://github.com/crustify-rs/ffibox.git /opt/ffibox`.
